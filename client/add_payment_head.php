...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($head_name) || $head_amount < 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO payment_heads (organisation_id, head_name, head_amount) VALUES (?, ?, ?)");
        
        if ($stmt) {
            // "isd" stands for Integer, String, Double (decimal)
            $stmt->bind_param("isd", $organisation_id, $head_name, $head_amount);
            
            if ($stmt->execute()) {
                $success = "Payment Head added successfully!";
            } else {
                $error = "Failed to add payment head: " . $stmt->error;
            }
            $stmt->close();
        } else {
...
<head>
    ...
    <title>Add Payment Head - Rotary Membership</title>
    ...
    <div class="dashboard-content">
        ...
        <div class="card">
            <div class="card-header">
                <h1><i class="fas fa-plus-circle"></i> Add Payment Head</h1>
                <p>Create a new fee category for your members.</p>
            </div>
            
            <form action="" method="POST">
                <div class="form-group">
                    <label for="head_name"><i class="fas fa-tag"></i> Head Name *</label>
                    <input type="text" name="head_name" id="head_name" required placeholder="e.g. Annual Subscription, Joining Fee">
                </div>

                <div class="form-group">
                    <label for="head_amount"><i class="fas fa-rupee-sign"></i> Amount *</label>
                    <input type="number" name="head_amount" id="head_amount" step="0.01" min="0" required placeholder="e.g. 1500.00">
                </div>

                <div class="form-actions">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Save Payment Head
                    </button>
