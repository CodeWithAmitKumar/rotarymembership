...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($member_id) || empty($first_name) || empty($last_name)) {
        ...
        // 1. Check for duplicate email (if an email was provided)
        if (!empty($email)) {
        ...
        // 2. If email is unique (or empty), proceed with inserting
        if (!$email_exists) {
            $query = "INSERT INTO members (organisation_id, member_id, first_name, last_name, joining_date, original_rotary_date, address, city_state, postal_code, phone_no, email, online_account_with_rotary, age_data_available, satellite_member) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if ($stmt) {
                $stmt->bind_param("isssssssssssss", 
                    $organisation_id, $member_id, $first_name, $last_name, $joining_date, 
                    $original_rotary_date, $address, $city_state, $postal_code, $phone_no, 
                    $email, $online_account_with_rotary, $age_data_available, $satellite_member
                );
                ...
                try {
                    ...
                    if ($e->getCode() == 1062) {
                        $error = "A member with this Rotary ID already exists!";
                    } else {
...
<head>
    ...
    <title>Add Member - Rotary Membership</title>
    ...
    <div class="dashboard-content">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <p>Register a new member to your organization.</p>
            </div>
            
            <form action="" method="POST">
                <div class="form-group">
                    <label for="member_id"><i class="fas fa-id-card"></i> Rotary Member ID *</label>
                    <input type="text" name="member_id" id="member_id" required placeholder="e.g. 12345678">
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="email" name="email" id="email" placeholder="member@example.com">
                    </div>
                    <div class="form-group">
                        <label for="phone_no"><i class="fas fa-phone"></i> Phone Number</label>
                        <input type="tel" name="phone_no" id="phone_no" placeholder="+1 234 567 8900">
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="original_rotary_date" id="original_rotary_date">
                    </div>
                </div>

                <div class="form-group">
                    <label for="address"><i class="fas fa-map-marker-alt"></i> Address</label>
                    <textarea name="address" id="address" placeholder="Full street address"></textarea>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label for="city_state"><i class="fas fa-city"></i> City / State</label>
                        <input type="text" name="city_state" id="city_state" placeholder="e.g. New York, NY">
                    </div>
                    <div class="form-group">
